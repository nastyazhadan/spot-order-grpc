...
func New(pool *pgxpool.Pool, logger *zapLogger.Logger, cfg config.SpotConfig) *OutboxStore {
	return &OutboxStore{
		pool:   pool,
		logger: logger,
		config: cfg,
...
func (s *OutboxStore) MarkPublished(ctx context.Context, event models.OutboxEvent) error {
	...
	metrics.ObserveWithTrace(ctx,
		metrics.DBQueryDuration.WithLabelValues(s.config.Service.Name, "spot.outbox.mark_published"),
		time.Since(start).Seconds(),
	)
...
func (s *OutboxStore) ScheduleRetry(
	...
	metrics.ObserveWithTrace(ctx,
		metrics.DBQueryDuration.WithLabelValues(s.config.Service.Name, "spot.outbox.schedule_retry"),
		time.Since(start).Seconds(),
	)
...
func (s *OutboxStore) MarkFailed(ctx context.Context, event models.OutboxEvent, errText string) error {
	...
	metrics.ObserveWithTrace(ctx,
		metrics.DBQueryDuration.WithLabelValues(s.config.Service.Name, "spot.outbox.mark_failed"),
		time.Since(start).Seconds(),
	)
...
func (s *OutboxStore) ReleaseStuckEvents(ctx context.Context, stuckBefore time.Time) (int64, error) {
	...
	metrics.ObserveWithTrace(ctx,
		metrics.DBQueryDuration.WithLabelValues(s.config.Service.Name, "spot.outbox.release_stuck"),
		time.Since(start).Seconds(),
	)
